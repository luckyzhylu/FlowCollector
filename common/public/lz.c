 ...
 * 作者： 李全兵
 */
unsigned char *Compress(unsigned char *in, int insize, int *outsize)
{
	unsigned int *work;
    int outbufsize = (insize * 257) / 256 + 1 + 4;
	unsigned char * buff = (unsigned char *)malloc(outbufsize);
	...
	if (work)
	{
		out = build_int(out, &outbufsize, insize);		
 ...
 * 参数：
 * 返回：
 * 功能： 对数据头进行处理后，解压缩数据内容
 * 作者： 李全兵
 */
unsigned char *Uncompress(unsigned char *in, int insize, unsigned int *outsize)
{	
	unsigned int oldsize = 0;
	unsigned char *out = NULL;

	// 移动in的指针，insize减4，oldsize存储in前4位转换后的值
	in = parse_int(in, &insize, &oldsize);
	...
	out = (unsigned char *)malloc(oldsize);
	if (out == NULL)
	{
		return NULL;
	}

	*outsize = LZ_Uncompress(in, out, insize, oldsize);
	if (*outsize == oldsize)
	{	
		return out;
	}
